// app.js — SpendLog SPA Application
// Teller bank integration — no login required, opens straight to calendar.
...
function showTxnModal(txn) {
  ...
  document.getElementById('modal-details').innerHTML = details.map(([l,v]) =>
    `<div class="modal-detail-row"><span class="modal-detail-label">${l}</span><span class="modal-detail-value">${v}</span></div>`
  ).join('');
...
async function loadAccounts() {
  ...
  try {
    State.accounts = await apiFetch('/teller/accounts');
  } catch(e) {
    State.accounts = [];
  }
  renderAccounts();
  ...
  container.innerHTML = State.accounts.map(acc => `
    <div class="account-card">
      ...
      <div class="account-body">
        ...
        ${acc.last_synced_at ? `<div class="account-mask">Last synced: ${formatDateShort(acc.last_synced_at.slice(0,10))}</div>` : ''}
      </div>
    </div>
  `).join('') + `
    ...
    <div class="security-note">
      ...
      <p>SpendLog never stores your bank credentials. Secured by Teller with bank-grade encryption.</p>
    </div>
  `;
  ...
  try {
    ...
    const teller = TellerConnect.setup({
      ...
      onSuccess: async (enrollment) => {
        ...
        try {
          const result = await apiFetch('/teller/enroll', {
            method: 'POST',
            body: JSON.stringify({
          ...
          alert('Failed to save bank connection: ' + e.message);
      ...
      onFailure: (failure) => {
        alert('Bank connection failed: ' + (failure.message || 'Unknown error'));
        btn.textContent = 'Connect a Bank'; btn.disabled = false;
      },
    });
    ...
    alert('Failed to load bank connection: ' + e.message);
    btn.textContent = 'Connect a Bank'; btn.disabled = false;
  }
}
...
function categoryEmoji(c) {
  ...
  if (l.includes('travel')||l.includes('transport')) return '✈️';
  if (l.includes('shop')||l.includes('retail')) return '🛍️';
  if (l.includes('health')||l.includes('medical')) return '💊';
  if (l.includes('entertain')||l.includes('recreation')) return '🎬';
  if (l.includes('service')||l.includes('utilities')) return '🔧';
  if (l.includes('transfer')) return '↔️';
  if (l.includes('grocery')||l.includes('supermarket')) return '🛒';
  if (l.includes('coffee')||l.includes('cafe')) return '☕';
  if (l.includes('gas')||l.includes('fuel')) return '⛽';
  return '💳';
}
function categoryBg(c) {
  if (!c) return 'rgba(144,144,176,0.12)'; const l=c.toLowerCase();
  if (l.includes('food')||l.includes('restaurant')||l.includes('dining')) return 'rgba(249,115,22,0.15)';
