...
// No login required — opens straight to calendar on install.
...
// ── Device ID — unique per install, no login needed ───────────────────────────
...
async function bootApp() {
  ...
  showApp(params.get('view') || 'calendar');
...
async function loadActivity(reset=true) {
  ...
  try {
    ...
    if (!State.transactions.list.length) {
      list.innerHTML = '<div class="empty-state"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg><p>No transactions yet</p><p>Connect a bank to get started</p></div>';
      return;
    }
    const groups = {};
...
function renderTxnItem(txn) {
  ...
  return `<div class="txn-item" data-id="${txn.id}"><div class="txn-icon" style="background:${categoryBg(txn.category)}">${categoryEmoji(txn.category)}</div><div class="txn-body"><div class="txn-name">${txn.merchant_name||txn.name}${badges?`<span class="txn-badges">${badges}</span>`:''}</div><div class="txn-meta">${txn.category||'Uncategorized'}</div></div><div class="txn-amount ${txn.pending?'pending':''}">${formatCurrency(txn.amount)}</div></div>`;
}
...
function showTxnModal(txn) {
  ...
  const details = [['Date',formatDate(txn.transaction_date)],['Category',txn.category||'Uncategorized'],txn.subcategory?['Subcategory',txn.subcategory]:null,['Payment',txn.payment_channel||'Card'],txn.is_apple_pay?['Wallet',' Apple Pay']:null,['Status',txn.pending?'⏳ Pending':'✅ Posted']].filter(Boolean);
...
async function loadAccounts() {
  ...
  try { State.accounts = await apiFetch('/plaid/accounts'); } catch(e) { State.accounts = []; }
  renderAccounts();
}
...
function renderAccounts() {
  ...
  `).join('') + `
    ...
    <div class="security-note">
      ...
      <p>SpendLog never stores your bank credentials. Secured by Plaid with 256-bit encryption.</p>
    </div>
  `;
...
async function startPlaidLink() {
  ...
  try {
    ...
    if (!window.Plaid) await loadScript('https://cdn.plaid.com/link/v2/stable/link-initialize.js');
    window.Plaid.create({
      token: data.link_token,
      onSuccess: async (publicToken, metadata) => {
        await apiFetch('/plaid/exchange-token', {method:'POST', body:JSON.stringify({public_token:publicToken,institution_name:metadata.institution?.name,institution_id:metadata.institution?.id,account_name:metadata.accounts?.[0]?.name})});
        await loadAccounts();
        State.calendar.days = {};
    ...
    alert('Failed to connect bank: ' + e.message);
...
function categoryBg(c) {
  ...
  if (l.includes('travel')) return 'rgba(59,130,246,0.15)'; if (l.includes('shop')) return 'rgba(168,85,247,0.15)';
  if (l.includes('health')) return 'rgba(239,68,68,0.15)'; if (l.includes('entertain')) return 'rgba(236,72,153,0.15)';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('txn-modal').addEventListener('click', e => { if(e.target===document.getElementById('txn-modal')) document.getElementById('txn-modal').classList.remove('show'); });
  document.getElementById('enable-notif-btn')?.addEventListener('click', async () => { if(await Notification.requestPermission()==='granted') subscribeToPush(); });
  document.getElementById('activity-scroll').addEventListener('scroll', e => { const el=e.target; if(el.scrollTop+el.clientHeight>=el.scrollHeight-100&&State.transactions.hasMore) loadActivity(false); });
  bootApp();
});
